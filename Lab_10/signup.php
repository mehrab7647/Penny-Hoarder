...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // TODO 6a: write a SQL query that finds matches for the username in the Loggers table
    $query = "SELECT * FROM Loggers WHERE username = :username";
    
    // TODO 6b: use PDO::query() to run the query
    $stmt = $db->prepare($query);
    $stmt->execute([':username' => $username]);

    // TODO 6c: use PDOStatement::fetch() to get the value of the first result of the query
    $match = $stmt->fetch();
    ...
    if (empty($errors)) {
        ...
        // TODO 7a: use PDO::exec() with an INSERT INTO statement to store the user's details in the Loggers table.
        //          Hint: Use "avatar_stub" as a temporary value for avatar_url.
        $query = "INSERT INTO Loggers (first_name, last_name, username, password, dob, avatar_url) VALUES (:first_name, :last_name, :username, :password, :dob, 'avatar_stub')";
        ...
        if (!$result) {
            ...
            // TODO 7b: Use lastInsertId() to get the uid from the last insert query.
            $uid = $db->lastInsertId();
            
            // TODO 7c: Rename the user's image to "uploads/uid.filetype" e.g: "uploads/12.jpg"
            ...
            if ($uploadOk) {
                ...
                if (!$fileStatus) {
                    ...
                    $errors["Server Error"] = "Failed to upload avatar.";
                    $uploadOK = FALSE;
                }
            }
            ...
            if (!$uploadOk)
            {
                ...
                $result = $db->prepare($query)->execute([':user_id' => $uid]);
                if (!$result) {
                    $errors["Database Error"] = "could not delete user when avatar upload failed";
                }
                $db = null;
            } else {
                // TODO 7d: use PDO::exec() to UPDATE the user's avatar_url field 
                //          in the Loggers table so that we can easily use it.
                // TODO 7d: Use PDO::exec() to UPDATE the user's avatar_url field in the Loggers table
                $query = "UPDATE Loggers SET avatar_url = :avatar_url WHERE user_id = :user_id";
                $result = $db->prepare($query)->execute([':avatar_url' => $target_file, ':user_id' => $uid]);
